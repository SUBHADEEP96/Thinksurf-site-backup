...
<head>
  ...
  <title>Euresia Decor - Production Details</title>
      ...
      <div id="content">
       ...
       <!--  <form class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800 text-center">Production</h1>
          <hr>
            ...
            <div class="col-lg-12">
             ...
             <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary text-center">View Production Details</h6>
                </div>
...
<?php
$mysqli= new mysqli('localhost','root','','euresia');

$result=$mysqli->query("SELECT item,descrip FROM design");

?>

  <div class="" style="font-size: 11px;">
  <form action="" method="POST" ><br> 
  <!--<input type="text" name="item" placeholder="" ><br>-->
  <div class="" >
  <select name="item" style="width:170px; height:200px;" class="form-control" id="framework" name="framework[]" multiple class="form-control">
    <option style="font-size:14px; background-color:#e6e6e6;font-weight:bold;">Select Design Id</option>
  ...
  while($row = $result->fetch_assoc())
  {
   $item=$row['item'];
   //$descrip=$row['descrip'];
   echo "<option style='font-size:12px'>$item</option>";
  }
  ?>

  </select>
  </div>
  <input type="submit" name="search" value="Submit" class="btn btn-primary btn-sm" style="width: 80px;">
  <br>
  ...
  if(isset($_POST['search']))
{
  ...
  $query = "SELECT * FROM design where item='$item' ";
...
<td>
  <a href="" class="" data-toggle="modal" data-target="#<?= $row['item']?>">
  <img src="img/<?php echo $row['uploadcad']?>" style="height:40px; width:40px"></a>
</td>

 <!-- The Modal -->
  <div class="modal" id="<?= $row['item']?>">
    <div class="modal-dialog">
      <div class="modal-content">
      
        <!-- Modal Header -->
        <div class="modal-header">
          <h4 class="modal-title">Item id: <?php echo $row['item']; ?></h4>
          <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
        
        <!-- Modal body -->
        <div class="modal-body">
          <img src="img/<?php echo $row['uploadcad']?>" class="mx-auto d-block img-thumbnail">
        </div>
...
<td>
  <a href="" class="" data-toggle="modal" data-target="#<?= $row['id']?>">
  <img src="img/<?php echo $row['uploadimg']?>" style="height:40px; width:40px"></a>
</td>
  ...
  <div class="modal" id="<?= $row['id']?>">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <!-- Modal body -->
        <div class="modal-body">
          <img src="img/<?php echo $row['uploadimg']?>" class="mx-auto d-block img-thumbnail">
        </div>

        
<!-- Modal footer -->
<div class="modal-footer" data-toggle="modal" data-target="#<?= $files['id']?>">
<button type="button" class="btn btn-primary">Download</button>
</div>
      </div>
    </div>
        ...
        <td><?= $row['threadcolor']?>,<?= $row['threadcolor1']?></td>
        <td><?= $row['piping']?> - <?= $row['pipingdetails']?></td>
        <td><?= $row['nostiches']?></td>
        <td><?= $row['rate']?></td>


<td> <input type="text" name="date" class="form-control-sm" style="width:155px" value="<?php echo date('jS,F Y');?>"></td>
<td> <input type="text" name="quantity" class="form-control-sm" placeholder="Quantity" style="width:80px"></td>
<td> <input type="text" name="item" class="form-control-sm" placeholder="Confirm Design Id" style="width:80px"></td>
...
$db=mysqli_select_db($con,"euresia");

if(isset($_POST['insert'])) 
{
  $item=$_POST['item'];

  $query="UPDATE design SET  date1='$_POST[date]', quantity='$_POST[quantity]' WHERE item='$_POST[item]' ";
  $query_run=mysqli_query($con,$query);
...
<input type="submit" class="btn btn-primary btn-user btn-block"  value="Submit" id="" name="insert">
</div>
